...
export default async function Backlog({ params }: ProjectOverviewPageProps) {
  ...
  const issues = await getIssuesJoined([and(eq(project.ID, params.id), isNull(issue.SprintID))]);
  ...
  const disableAddingSprint = currentSprints.filter(s => s.EndDate > new Date()).length > 0;
  ...
  return (
    <div className="flex flex-col">
		<div className='flex flex-row'>
			<CreateIssueDialog
				trigger={<button disabled={disableAddingSprint} className={cn("flex cursor-pointer ml-4 items-center justify-between rounded-md px-4 py-2 text-center text-sm font-semibold uppercase text-white transition duration-200 ease-in-out hover:bg-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-600 focus-visible:ring-offset-2 active:scale-95", (disableAddingSprint ? "bg-slate-200" : "bg-gray-600") )}>New Sprint</button>}
				issues={issues}
				user={user}
				projectId={projectId}
				newSprintNumber={Number(newSprintNumber)}
			/>
			{disableAddingSprint && <p className="ml-4 mt-2">There is an ongoing sprint in this project. You cannot create new one</p>}
		</div>
      <SprintsComponent
        issues={grouped}
        users={users}
